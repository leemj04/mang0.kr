...
소인수 분해, 이산 로그 문제 등은 Abelian group에 대한 HSP로 환원되고, 이는 quantum polynomial time으로 해결 가능하다. 하지만 LWE는 $e$라는 noise 때문에 주기적 구조가 없고, Abelian HSP 기반 양자 알고리즘을 직접 적용할 수 없다.
...
네 번째 레지스터의 값을 측정한 결과 $\mathbf{y} \in \mathbb{Z}_q^m$을 얻었다고 하자. $\mathbf{y} = \mathbf{As}-j\mathbf{e}_0 + \mathbf{x} \bmod q$ 이므로, $\mathbf{x} \equiv \mathbf{y} - (\mathbf{As}-j\mathbf{e}_0) \bmod q$가 되고, $\mathbf{x}$는 $(\mathbf{s}, j)$에 의해 결정된다는 것을 알 수 있다. 네 번째 레지스터를 관측했으므로 state는 아래와 같이 된다.
$$ \sum_{\mathclap{\substack{
    \mathbf{s} \in \mathbb{Z}_q^n \\
    j \in \mathbb{Z} \cap [-\sqrt{\kappa}r,\sqrt{\kappa}r]
}}}
...
이므로, $\ket{b}$의 진폭은 $\displaystyle \sum_{j\in\mathbb Z_q} e^{2 \pi i \cdot j \cdot (\braket{\mathbf{a}_k, \mathbf{s}_0} + b)/q} \cdot \rho_r(j)$이 된다. 이제 $\mathbf{u} = \dfrac{\braket{\mathbf{a}_k, \mathbf{s}_0} + b}{q}$로 두면, 1차원 Poisson 형태가 되고, 식을 아래와 같이 변형할 수 있다.
